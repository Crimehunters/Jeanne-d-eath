...
var speed
export var default_boss_hp = 140
var hp
enum CharFace{Right, Left}
var attacking = false
var direction = Vector2.ZERO
export var default_atk_cooldown = 1.25
var atk_cooldown
var ply
var TargetHitboxAry = []
...
func apply_ability(delta):
	if(skill_cd > 0): skill_cd -= delta
		...
		if(ai_state == ai_states.OFFENCE): SkillPool.use(11, "Boss")
		elif(ai_state == ai_states.DEFFENCE): SkillPool.use(13, "Boss")
		skill_cd = 8
...
func summon_atk():
	var crossedball_ins = load("res://scenes/BossNA/CrossedBall.tscn").instance()
	get_parent().add_child(crossedball_ins)
